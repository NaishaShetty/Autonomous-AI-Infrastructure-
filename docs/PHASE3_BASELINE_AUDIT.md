...
## 26. Phase 3.10 data sufficiency and decision-time observability audit

Phase 3.10 audited the actual restored Alibaba GPU2020 data and processed research boundary without training a new model or creating a V1.1 candidate. The main sample contains 10,000 terminal jobs and a 14-feature numeric V1 contract, with 16 global task names and 5 observed GPU types. The canonical temporal and Phase 3.5 future folds show meaningful failure-rate and workload variation, but the benchmark remains a later slice of one restored trace rather than verified independent environments.

The audit established that V1's pre-outcome workload/resource aggregates are present and represented, while the exact runtime prediction timestamp, synchronized ingestion/scheduling/allocation timestamps, queue pressure, live cluster state, node health, network state, operational consequence severity, and complete environment provenance are absent or unproven. No potentially useful unused field was promoted to clean decision-time status because timestamp quality was insufficient. Post-outcome telemetry and final status remain forensic-only.

The structured scorecard is: decision-time observability **PARTIAL**; temporal coverage **PARTIAL**; failure diversity **PARTIAL**; workload diversity **PARTIAL**; environment diversity **UNKNOWN**; provenance **PARTIAL**; timestamp quality **INSUFFICIENT**; generalization support **PARTIAL**. The three-hypothesis evidence matrix supports a combined **B + C** interpretation: missing or poorly timestamped decision-time information and a narrow evaluation/environment boundary both limit scientific progress. The final decision is **V1 LIMITATIONS APPEAR DATA-BOUND**. No V1.1 integration or model experiment is authorized until rigorous observability/data collection is completed.

Complete Phase 3.10 artifacts, including the data inventory, timestamp audit, temporal dependency graph, decision-time information matrix, missingness, completeness, dependence, diversity, distribution-shift, bottleneck, scorecard, hypothesis matrix, synthesis, plots, and immutable hashes, are stored under `experiments/results/v1_1/data_sufficiency_audit/3_10/`. The current full-suite attempt reached approximately 47% and timed out after five minutes with exit code `124`; the inherited verified 558-passed / 7-skipped result remains distinct. Phase 3.10 focused tests passed with 9 tests.

## 27. Phase 3.11 + 3.12 decision-time observability and multi-environment data foundation

The combined phase implemented an additive, versioned data foundation without modifying frozen V1, retraining any model, creating V1.1, changing canonical splits, merging external data, or overwriting historical evidence. The foundation provides canonical event, decision-time snapshot, timestamp-quality, provenance, environment identity, dataset identity, workload, resource state, prediction, failure, recovery, operational-consequence, adapter, registry, coverage, quality-gate, contamination, and future holdout-split contracts.

The current Alibaba GPU2020 source remains the canonical control source with metadata-only normalization. It is registered as one environment/trace. Additional sources were not silently acquired or integrated; they remain deferred until provenance, licensing, timestamp quality, decision-time support, schema compatibility, and reproducibility can be evaluated. Environment, environment-by-temporal, temporal, and workload-family holdout contracts are implemented as contract-only infrastructure and were not used to train a model.

The readiness gate is: observability foundation **PARTIAL**; multi-environment foundation **PARTIAL**; data provenance **PARTIAL**; timestamp integrity **PARTIAL**; environment identity **PARTIAL**; multi-regime splits **READY — CONTRACTS ONLY**; data quality **PARTIAL**; benchmark foundation **PARTIAL**. The exact runtime prediction timestamp and synchronized live scheduler/resource context remain unavailable in the current source. The next-phase decision is **D — BOTH INCOMPLETE**. The required next step is real timestamped observability collection and registration of at least one genuinely independent environment before any new V1.1 model experiment.

Complete Phase 3.11+3.12 artifacts are isolated under `experiments/results/v1_1/data_foundation/3_11_3_12/`, including schemas, registries, source acceptance, normalization mapping, split contracts, quality reports, coverage, plots, reports, synthesis, and immutable SHA-256 hashes. Focused foundation tests passed with 10 tests. The current full repository suite reached approximately 46% and timed out after five minutes with exit code `124`; this incomplete current run is distinct from the inherited verified 558-passed / 7-skipped result.

## 28. Phase 4.0 + 4.1 architecture, system contract, and runtime observability

Phase 4.0 architecture and system contracts were implemented and gated before Phase 4.1 runtime work began. The architecture defines explicit boundaries for observability, detection, prediction, decision/safety, diagnosis, failure memory, recovery planning, recovery execution, validation, learning, and orchestration. It reuses the Phase 3.11+3.12 event, timestamp, decision-time, and provenance contracts rather than creating incompatible taxonomies.

The architecture enforces `PREDICTION → DECISION POLICY → SAFETY GATE → AUTHORIZED ACTION → EXECUTOR`. A model, planner, or language-model component cannot directly mutate infrastructure. Recovery execution is distinct from recovery validation, with explicit `RECOVERED`, `NOT_RECOVERED`, and `UNKNOWN` outcomes. Invalid input, schema, timestamp, provenance, missing evidence, stale state, unavailable observation, unsafe action, execution, validation, unknown-outcome, and internal failures remain distinct error semantics.

The Phase 4.0 gate passed: architecture **READY**; interfaces **READY**; state machine **READY**; decision-time integrity **READY**; provenance **READY**; safety boundary **READY**; focused contract tests **PASS**.
...
## 30. Phase 4.1.1 runtime source feasibility and instrumentation decision

Phase 4.1.1 audited the repository for a legitimate timestamped runtime event source before Phase 4.2. Existing canonical events, decision-time snapshots, provenance, timestamp validation, persistent storage, replay, and bounded API infrastructure were confirmed reusable. No project-owned workload executor, approved simulator with real event semantics, live telemetry producer, scheduler source, or synchronized external runtime event stream was identified.

Alibaba GPU2020 remains a **HISTORICAL_TRACE**: replayable processed data with partial source timestamps, but not a synchronized runtime source. The persistent event store is **STORAGE INFRASTRUCTURE ONLY**, not a source. Synthetic fixtures remain **SYNTHETIC_TEST_FIXTURE** and are engineering tests only. No fake source, timestamp, queue/resource state, environment identity, or benchmark evidence was created.

The final source decision is **D — NO LEGITIMATE RUNTIME SOURCE CURRENTLY AVAILABLE**. The machine-readable feasibility artifact records source class, engineering status, research status, timestamp quality, decision-time support, environment identity, provenance, reproducibility, acceptance decision, reasons, and limitations. The final gate is partial or unavailable for runtime collection, decision-time support, resource/scheduler observability, and independent environment identity; persistent integration and replay are ready for a future legitimate source; V1 and historical protection pass.

**PHASE 4.2 AUTHORIZATION: NOT AUTHORIZED.** A future phase may revisit this decision only after an explicitly approved real or controlled runtime process produces actual event occurrences with known timestamp semantics, provenance, environment identity, and reproducible persistence. New evidence is isolated under `experiments/results/v1_1/phase4_source_feasibility/4_1_1/`; no model, detector, diagnosis, recovery, benchmark, or V1.1 implementation was added.

## 31. Phase 4.1.2 controlled runtime and runtime instrumentation

Phase 4.1.2 created the first legitimate project-owned runtime observation source. A minimal local Python controlled runtime launches actual subprocess workloads and emits canonical events through the existing `ObservationCollector` and `PersistentEventStore`. It does not create a second database or event taxonomy, and it does not implement Phase 4.2 detection, diagnosis, recovery, V1.1, model training, benchmark construction, or production deployment.

The runtime recorded actual workload receipt, registration, execution start, live `/proc` telemetry, successful completion, nonzero process exit, and actual timeout termination. Three engineering scenarios passed: a successful process completed with exit code 0; an intentionally failing process exited with code 7 and emitted `failure_detected`; and a sleeping process exceeded the configured deadline, was actually killed, and emitted a timeout failure event. Restart and deterministic replay passed over the same persistent SQLite store.

The source is registered explicitly as **CONTROLLED_RUNTIME**, project-owned, local, reproducible by configuration, and limited to one controlled environment identity. Local process CPU/RSS telemetry is observed. GPU, scheduler, queue, cluster allocation, node-level scheduling, recovery, validation, and production consequence fields are **UNAVAILABLE** and were not fabricated. Timestamps are actual UTC-aware local runtime-boundary timestamps, exact for that boundary but not synchronized external-infrastructure timestamps.
