...
class JsonExperienceStore:
    ...
    def _lineage(episode: RuntimeEpisode) -> TemporalLineage:
        grouped: dict[RuntimeState, list[Any]] = {}
        for transition in episode.transitions:
            grouped.setdefault(transition.to_state, []).append(transition.timestamp)
        first = lambda state: min(grouped[state]) if state in grouped else None
        last = lambda state: max(grouped[state]) if state in grouped else None
        return TemporalLineage(observation_ts=episode.observation.timestamp, detection_ts=first(RuntimeState.DETECTED), diagnosis_ts=first(RuntimeState.DIAGNOSED), recovery_decision_ts=first(RuntimeState.RECOVERY_PLANNED), recovery_execution_ts=first(RuntimeState.RECOVERY_EXECUTING), validation_ts=last(RuntimeState.RECOVERY_VALIDATING), outcome_ts=utcnow())
