...
tags:
  ...
  - evaluation
  - research
pretty_name: Autonomous AI Infrastructure — Phase 5.3/5.4 Benchmark
size_categories:
  - 1K<n<10K
---
...
**License: MIT** for this benchmark implementation (runner, task
registry, metrics, baselines, ablations, validation/leakage/reproducibility
tooling) — see `LICENSE` in this directory. Decided by the project owner
on 2026-08-27 after an upstream license/provenance audit
(`LICENSE_PROVENANCE_AUDIT.md`, Phase 5.6 release directory) found no
restriction on redistribution: all code here is self-authored. The
bundled dataset copy under this package's `experiments/` directory is
dataset content, not software, and is licensed separately under
**CC BY 4.0**, not MIT — see `DATASET_CARD.md`'s equivalent disclosures
under the paired dataset package. The dataset itself is derived
exclusively from this project's own Gen-3 evidence — no third-party
