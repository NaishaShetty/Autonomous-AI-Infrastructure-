...
content is included. **License: CC BY 4.0** (Creative Commons Attribution
4.0 International) — see `LICENSE` in this directory. The project owner
made this explicit decision on 2026-08-27, after an upstream license/
provenance audit (`LICENSE_PROVENANCE_AUDIT.md` in the Phase 5.6 release
directory) confirmed no upstream source restricts redistribution: no raw
third-party dataset text is stored in any record (sentiment/QA records
store only model checkpoint names and derived confidence scores, not
input text); the evaluation corpora are self-authored templates, not
copied from GLUE/SST-2/SQuAD; the two Hugging Face model checkpoints used
to produce derived scores (`distilbert-base-uncased-finetuned-sst-2-english`,
`distilbert-base-cased-distilled-squad`) are both Apache-2.0 with no
restriction on redistributing derived outputs; and the historical
AgentRx/Alibaba GPU2020/AIOps 2020 datasets used in earlier project phases
are confirmed absent from this canonical dataset. You are free to share
and adapt this dataset for any purpose, including commercially, provided
you give appropriate attribution (see `CITATION.cff`) — see `LICENSE` for
the full terms.
