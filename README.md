...
> see [`#phase3-freeze`](#phase3-freeze).

> **All project documentation (every phase, historical and active) now
> lives in this file, below.** What used to be the separate
> `docs/PROJECT_HISTORY.md` — assembled by concatenating each original
> phase document verbatim behind a status banner (FROZEN HISTORICAL /
> ACTIVE / PLANNING), with a table of contents and a timeline explaining
> how the phases relate — is merged in starting at
> ["Project History — Consolidated Documentation"](#project-history--consolidated-documentation)
> below; see that section's own "How to read this document" subsection.
> The individual per-phase files that used to live under `docs/` and at
> the repo root (`PHASE1_AUDIT_REPORT.md`, `PHASE2_REPORT.md`,
> `PROJECT_HISTORY.md`) have been removed; their content was not altered,
> only relocated into this file.

> **Latest milestone: ACTIVE Phase 4.2 (failure pattern learning on real
> data) is complete — verdict INCONCLUSIVE.** Full protocol, implementation,
> experiments, ablations, leakage audit, and final verdict are merged in
> starting at
> ["ACTIVE Phase 4.2 — Failure Pattern Learning (Real-Data Foundation)"](#active-phase-42--failure-pattern-learning-real-data-foundation)
> below (what used to be the separate `docs/PHASE4_2_ACTIVE_FAILURE_PATTERNS.md`).
> The approved research plan that authorized this milestone is preserved
> further up under "ACTIVE PHASE 4.2 PLAN", with the completion recorded
> as an additive amendment to the "PHASE4 PLAN" section (§12) — see the
...
[`#phase1-audit-report`](#phase1-audit-report)
for the source-prototype audit and
[`#phase2-report`](#phase2-report)
...
```
src/
  schema/        canonical ReliabilityEvent (pydantic) — see #schema
  ...
  experience/    old (frozen) Phase 4.1 -- synthetic-only retrieval-precision experience store, see #phase4-1-failure-memory
  patterns/      old (frozen) Phase 4.2 -- synthetic-only failure pattern learning, see #phase4-2-failure-patterns
  failure_experience/  active Phase 4.1 -- canonical FailureExperience schema/storage/retrieval across real+synthetic sources, see #phase4-1-active-failure-experience
  failure_patterns/  active Phase 4.2 -- failure-rate-elevation pattern discovery (Alibaba primary) + descriptive recurrence analysis (AIOps/AgentRx), built on failure_experience; see docs/PHASE4_2_ACTIVE_FAILURE_PATTERNS.md (verdict: INCONCLUSIVE, evidence-volume gate not met)
...
[`#phase3-1-evaluation-protocol`](#phase3-1-evaluation-protocol).
...
Full writeup: [`#phase3-2-representation-experiments`](#phase3-2-representation-experiments). Does not modify the Phase 3.1 protocol or any Phase 2 component; new representations live only in `src/evaluation/representations.py`, used for evaluation only (not wired into `src/decision` or the live API).
...
mocked — see [`#phase2-report`](#phase2-report) section 2).
...
[`docs/PHASE4_2_ACTIVE_FAILURE_PATTERNS.md`](#active-phase-42--failure-pattern-learning-real-data-foundation).
Consumes `src/failure_experience/` (active Phase 4.1) as its data layer;
does not modify or import the old, frozen `src/patterns/`
(`#phase4-2-failure-patterns`, also `INCONCLUSIVE`,
on unrelated synthetic data — that verdict is untouched by this milestone).
...
seeded into it (see [`#phase1-audit-report`](#phase1-audit-report)
...
<a id="project-history--consolidated-documentation"></a>
...
Phase 1 (prototype audit) -> Phase 2 (migration/integration)
  ...
  -> ACTIVE Phase 4.2 (src/failure_patterns/) -> INCONCLUSIVE (evidence-volume gate not met)
```
...
- [PHASE4 PLAN](#phase4-plan) — *ACTIVE (amended)* — The Phase 4 master plan. Sections 0-10 are the original frozen plan; Section 11 is an additive amendment covering the real-data-driven Phase 4 reboot; Section 12 is a further additive amendment recording ACTIVE Phase 4.2's completion. Nothing in sections 0-11 was rewritten.
...
- [PHASE4 2 ACTIVE PLAN](#phase4-2-active-plan) — *ACTIVE / CURRENT (plan; implemented)* — ACTIVE Phase 4.2 plan: reassessment and research plan for failure pattern learning on the post-real-data foundation. The plan below is preserved verbatim as approved; see `docs/PHASE4_2_ACTIVE_FAILURE_PATTERNS.md` for the implementation, experiments, and final verdict (INCONCLUSIVE).
...
## 12. Amendment — Active Phase 4.2 (failure pattern learning) completion

A further additive, independent package (`src/failure_patterns/`)
implements the **active Phase 4.2** milestone against the active Phase 4.1
`FailureExperience` substrate (§11 above) — see
[`docs/PHASE4_2_ACTIVE_FAILURE_PATTERNS.md`](#active-phase-42--failure-pattern-learning-real-data-foundation)
for the full protocol, implementation, experiments, ablations, leakage
audit, and formal verdict (**INCONCLUSIVE**, per a pre-registered
`minimum_evaluable_n` evidence-volume gate — evidence at the required
candidate granularity, 21 evaluable test-split contexts, did not clear the
frozen bar of 50). It does not import, edit, or extend-in-place
`src/patterns/` (old Phase 4.2, frozen, `INCONCLUSIVE` on synthetic data —
unaffected by this amendment) or `src/experience/` (old Phase 4.1). The
plan this milestone executed is preserved verbatim in the "ACTIVE PHASE 4.2
PLAN" section below it approved, unmodified by this amendment.

Nothing in §0–§11 above, nor in the frozen historical Phase 4.2 record
(`docs/PHASE4_2_FAILURE_PATTERNS.md`), is retroactively changed by this
amendment. Phase 4.3 onward (recovery policy learning, etc.) remains open
and unauthorized pending a further go-ahead.
